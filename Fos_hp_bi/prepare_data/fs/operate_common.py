from gateway.conf import FS_LISTDIR_URL, FS_EXISTS_URL, FS_UPLOAD_URL, FS_DOWNLOAD_URL, FS_DELETE_URL, FS_RENAME_URL,\
    FS_CHUNK_UPLOAD_URL, FS_CHUNK_UPLOAD_STATUS_URL, FS_PATH_META_URL
...
def get_path_meta(authorization, dest_path):
    logger.info(f"get dest_path: {dest_path}")
    headers = get_gateway_headers(authorization)
    params = {
        "path": dest_path
    }
    res = call_get_gateway_api(FS_PATH_META_URL, headers=headers, params=params, verify=False)
    if res.ok:
        result = res.json()
        if result['successful']:
