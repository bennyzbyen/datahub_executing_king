from common_utils.all_modules import pd, os, datetime, time, mlpapp, clickhouse_connect, hashlib, GateWayClient, insert_file, logger
from params_configs.db_config import clickhouse_token, clickhouse_connect_params, cluster

class DataStorage:
    """
    DataStorage 类：负责 boost5 业务线的数据清洗、多字段地理维度映射及 ClickHouse 高效上传。
    """
    def __init__(self, df_details: pd.DataFrame, df_summary: pd.DataFrame, df_ttl: pd.DataFrame, time_range: dict):
        ...
        # 临时文件名配置
        self.detail_file_path = "cal_b5_detail.csv"
        self.summary_file_path = "cal_b5_summary.csv"
        self.ttl_file_path = "cal_b5_ttl.csv"
    ...
    def _upload_2_clickhouse(self, df: pd.DataFrame, file_path: str, clickhouse_table: str, client):
        ...
        if clickhouse_table in ['store_assess_channel']:
            delete_query = f"ALTER TABLE {clickhouse_table} {cluster} DELETE WHERE sku_type = 'boost5'"
        else:
            delete_query = f"ALTER TABLE {clickhouse_table} {cluster} DELETE WHERE mars_week = '{mars_week}'"
        
        try:
            logger.info(f"Executing delete query on {clickhouse_table}...")
            ...
            logger.info(f"Successfully uploaded to {clickhouse_table}")
        finally:
    ...
    def run(self):
        ...
        # 2. 对三张核心表进行维度增强与上传
        tasks = [
            ('df_details', self.detail_file_path, "store_b5_sku_details"),
            ('df_summary', self.summary_file_path, "store_b5_sku_summary"),
            ('df_ttl', self.ttl_file_path, "store_b5_sku_ttl")
        ]
        ...
        for attr_name, file_path, table_name in tasks:
            ...
            if df is not None:
                logger.info(f"Enriching geo info and uploading to {table_name}...")
