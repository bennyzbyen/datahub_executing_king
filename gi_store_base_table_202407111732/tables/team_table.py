...
class TeamDim(object):
    def __init__(self, period: str, otc_list:list, douyin_customer_list: list, calendar_df: pd.DataFrame, cmt_geo_df: pd.DataFrame,
                 cmt_calculate_sellout_df: pd.DataFrame,cmt_segment_df: pd.DataFrame,cmt_customer_df: pd.DataFrame,cmt_inventory_df: pd.DataFrame,
                 cmt_sellin_df: pd.DataFrame,cmt_subsegment_df: pd.DataFrame,cmt_product_df: pd.DataFrame,logger: any):
        self.logger = logger
        self.Period = period
        ...
        self.douyin_customer_list = douyin_customer_list
        self.output_columns = [
            'update_time', 'last_update_time', 'period', 'team', 'customer_category',
    ...
    def get_inventory_date_id(self):
        ...
        pass
    def get_merge_columns(self,df):
        if 'GeographyID' in df.columns:
            return ['period','CustomerID','GeographyID']
        else:
    ...
    def handle(self):
        ...
        df = pd.merge(df,self.cmt_geo_df[['GeographyID', 'RegionCode', 'RegionName', 'ProvinceCode_G', 'ProvinceName_G',
                                'CitygroupCode', 'CitygroupName', 'CityCode_G', 'CityName_G']],how='left',on=['GeographyID'])
        ...
        df = self.team_and_customer_category(df)
    ...
    def team_and_customer_category(self, final_df: pd.DataFrame) -> pd.DataFrame:
        """
        "战队根据不同逻辑判断
        1. CGP ： 客户类型三级不等于“全国重点客户”，大区等于""CGP订单大区""
        2. NCD ： 客户类型三级不等于“全国重点客户”，大区等于""NCD 二包商订单大区""
        或者 ：客户类型三级等于“全国重点客户”，大区 in （”业务东区“，”业务南区“，”业务西区“，”业务北区“）
        3. 东区DTR：客户类型三级不等于“全国重点客户”，大区等于""业务东区""
        4. 南区DTR：客户类型三级不等于“全国重点客户”，大区等于""业务南区""
        5. 西区DTR：客户类型三级不等于“全国重点客户”，大区等于""业务西区""
        6. 北区DTR：客户类型三级不等于“全国重点客户”，大区等于""业务北区""
        7. 电商：【客户类型三级不等于“全国重点客户”，大区等于"电商大区"】 且 【客户编码 不包含抖音soldto（参考【抖音客户列表】）】
        8. 抖音：客户编码 包含抖音soldto（参考【抖音客户列表】A列的值=底表的customer_no）
        9. NCO: 客户类型三级不等于“全国重点客户”，大区等于""总部订单大区"

        :param final_df:
        :param douyin_customer_list:
        :return:
        """
        # team
        final_df['team'] = ''
        
        final_df.loc[final_df['customer_no'].isin(self.douyin_customer_list), 'team'] = '抖音'
        cond = final_df['customer_attribute'] == '全国重点客户'
        cond1 = final_df['mars_region_name'] == 'CGP订单大区'
        final_df.loc[~cond & cond1, 'team'] = 'CGP'

        cond2 = final_df['mars_region_name'] == 'NCD 二包商订单大区'
        final_df.loc[~cond & cond2, 'team'] = 'NCD'

        cond3 = final_df['mars_region_name'].isin(['业务东区', '业务南区', '业务西区', '业务北区'])
        final_df.loc[cond & cond3, 'team'] = 'NCD'

        cond4 = final_df['mars_region_name'] == '业务东区'
        final_df.loc[~cond & cond4, 'team'] = '东区DTR'

        cond5 = final_df['mars_region_name'] == '业务南区'
        final_df.loc[~cond & cond5, 'team'] = '南区DTR'

        cond6 = final_df['mars_region_name'] == '业务西区'
        final_df.loc[~cond & cond6, 'team'] = '西区DTR'

        cond7 = final_df['mars_region_name'] == '业务北区'
        final_df.loc[~cond & cond7, 'team'] = '北区DTR'

        cond8 = (final_df['mars_region_name'] == '电商大区') & (~final_df['customer_no'].isin(self.douyin_customer_list))
        final_df.loc[~cond & cond8, 'team'] = '电商'

        cond9 = final_df['mars_region_name'] == '总部订单大区'
        final_df.loc[~cond & cond9, 'team'] = 'NCO'


        # customer_category
        final_df['customer_category'] = ''

        # "客户分类
        # 1. DNKA: 客户类型三级等于“全国重点客户”，大区 in （”业务东区“，”业务南区“，”业务西区“，”业务北区“）
        # 2. DTR:
        # 客户类型三级不等于“全国重点客户”，大区等于""CGP订单大区""                               或
        # 客户类型三级不等于“全国重点客户”，大区等于""NCD 二包商订单大区""                     或
        # 客户类型三级不等于“全国重点客户”，大区等于""业务东区""                                     或
        # 客户类型三级不等于“全国重点客户”，大区等于""业务南区""                                       或
        # 客户类型三级不等于“全国重点客户”，大区等于""业务西区""                                       或
        # 客户类型三级不等于“全国重点客户”，大区等于""业务北区""                                          或
        # 客户类型三级不等于“全国重点客户”，大区等于""电商大区"" 且 客户编码  不包含抖音soldto（参考【抖音客户列表】）            或
        # 客户编码  包含抖音soldto（参考【抖音客户列表】】A列的值=底表的customer_no）                                                     或
        # 客户类型三级不等于“全国重点客户”，大区等于""总部订单大区"""

        cond = final_df['customer_attribute'] == '全国重点客户'
        cond1 = final_df['mars_region_name'].isin(['业务东区', '业务南区', '业务西区', '业务北区'])
        final_df.loc[cond & cond1, 'customer_category'] = 'DNKA'

        cond2 = final_df['customer_attribute'] != '全国重点客户'
        cond3 = final_df['mars_region_name'] == 'CGP订单大区'
        cond4 = final_df['mars_region_name'] == 'NCD 二包商订单大区'
        cond5 = final_df['mars_region_name'] == '业务东区'
        cond6 = final_df['mars_region_name'] == '业务南区'
        cond7 = final_df['mars_region_name'] == '业务西区'
        cond8 = final_df['mars_region_name'] == '业务北区'
        cond9 = (final_df['mars_region_name'] == '电商大区') & (~final_df['customer_no'].isin(self.douyin_customer_list))
        cond10 = final_df['customer_no'].isin(self.douyin_customer_list)
        cond11 = final_df['mars_region_name'] == '总部订单大区'

        final_df.loc[cond2 & cond3, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond4, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond5, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond6, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond7, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond8, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond9, 'customer_category'] = 'DTR'
        final_df.loc[cond10, 'customer_category'] = 'DTR'
        final_df.loc[cond2 & cond11, 'customer_category'] = 'DTR'
